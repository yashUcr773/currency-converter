...
export const UnitSelector = ({ 
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
          ...
          <CardContent className="p-3 sm:p-4 flex-1 flex flex-col items-center justify-center text-center space-y-2 sm:space-y-3">
            ...
            <div className="relative">
              ...
              <div className="relative w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center bg-primary/10 rounded-full border border-primary/20 shadow-sm group-hover:shadow-md group-hover:scale-105 transition-all duration-300">
                <Plus size={16} className="sm:w-5 sm:h-5 text-primary group-hover:text-primary-dark" />
              </div>
            </div>
            
            {/* Text content */}
            <div className="space-y-0.5 sm:space-y-1">
              <h3 className="font-bold text-xs sm:text-sm text-foreground group-hover:text-primary transition-colors duration-200">
                Add Unit
              </h3>
      ...
      <DialogContent className="max-w-sm mx-auto bg-card/95 backdrop-blur-md border-0 shadow-2xl rounded-2xl">
        <DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <Input
              ...
              className="pl-10 bg-background/50 border-2 border-border focus:border-primary focus:ring-4 focus:ring-primary/20 transition-all duration-200 rounded-xl h-12 text-base touch-manipulation"
              autoFocus
            />
          </div>
            ...
            {filteredUnits.length === 0 ? (
              ...
              filteredUnits.map((unit) => (
                <Button
                  key={unit.id}
                  variant="ghost"
                  className="w-full justify-start p-4 h-auto bg-card hover:bg-primary/5 border border-border hover:border-primary transition-all duration-200"
                  onClick={() => handleSelectUnit(unit)}
                >
                  <div className="flex items-center gap-3 w-full">
                    <div className="text-lg font-bold text-primary min-w-[3rem] text-left">
                      {unit.symbol}
                    </div>
